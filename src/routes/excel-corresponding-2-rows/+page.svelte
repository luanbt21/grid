          ...
          if (sumLeft > 0 && sumLeft === sumRight) {
            ...
            if (rowsWithLeft.length === rowsWithRight.length) {
              finalLeftRows = rowsWithLeft.map((leftRow, i) => ({
                ...leftRow,
                ...(ICODE && CODE
                  ? { [ICODE]: rowsWithRight[i]?.[CODE] || "" }
                  : {}),
              }));
              finalRightRows = rowsWithRight.map((rightRow, i) => ({
                ...rightRow,
                ...(ICODE && CODE
                  ? { [ICODE]: rowsWithLeft[i]?.[CODE] || "" }
                  : {}),
              }));
